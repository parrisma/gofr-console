 ...
 */
export const RESTRICTION_TEMPLATES_LIST = [
  {
    id: 'none',
...
export type TemplateId = (typeof RESTRICTION_TEMPLATES_LIST)[number]['id'];
...
export function applyTemplate(templateId: TemplateId): ClientRestrictions {
  const template = RESTRICTION_TEMPLATES_LIST.find((t) => t.id === templateId);
  return JSON.parse(JSON.stringify(template?.restrictions ?? TEMPLATE_NONE));
}
...
export function getTemplate(templateId: TemplateId) {
  return RESTRICTION_TEMPLATES_LIST.find((t) => t.id === templateId);
}
...
export function getAllTemplates() {
  return RESTRICTION_TEMPLATES_LIST;
}
