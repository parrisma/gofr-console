...
function summarizeArgs(args: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(args)) {
    if (/token|authorization|secret|password|api[_-]?key|cookie/i.test(key))
      continue;
    ...
    if (typeof value === "string") {
      entries.push([
        key,
        value.length > 120 ? `${value.slice(0, 120)}...[TRUNCATED]` : value,
      ]);
      continue;
    }
    if (
...
function getImageContent(
  ...
  if (!image?.data || !image?.mimeType) {
    throw new ApiError({
      ...
      recovery:
        "If proxy mode was enabled, call get_image with the returned GUID. Otherwise check the tool output for errors.",
    });
  }
...
function parseToolText<T>(
  ...
  try {
    ...
    const obj: ToolErrorShape | null =
      parsed && typeof parsed === "object" ? (parsed as ToolErrorShape) : null;
    ...
    if (serverErr) {
      ...
      throw new ApiError({
        ...
        message: serverErr,
        recovery:
          "This is a server-side bug — the MCP tool returned an error string instead of JSON. Check server logs.",
      });
    }
    ...
    throw new ApiError({
      ...
      recovery:
        "The MCP tool returned non-JSON text. Check server logs for errors.",
      cause: err,
    });
  }
}

function extractNpErrorInfo(
  data: unknown,
): { error: string; detail?: string; recovery?: string } | null {
...
async function callNpToolWithAuthFallback<T>(
  ...
  const firstArgs = explicitAuthToken
    ? applyAuthToArgs(args, explicitAuthToken)
    : args;
  const first = await client.callTool<HealthCheckResult>(
    toolName,
    firstArgs,
    explicitAuthToken,
  );
  const firstText = getTextContent(first, "gofr-np", toolName);
  const firstData = parseToolText<T | NpErrorResponse>(
    "gofr-np",
    toolName,
    firstText,
  );
  ...
  if (explicitAuthToken) {
    throw new ApiError({
      ...
      message: firstErr.detail
        ? `${firstErr.error}: ${firstErr.detail}`
        : firstErr.error,
  ...
  if (/^AUTH_REQUIRED\b/i.test(firstErr.error)) {
    ...
    if (token) {
      ...
      const second = await client.callTool<HealthCheckResult>(
        toolName,
        secondArgs,
        token,
      );
      const secondText = getTextContent(second, "gofr-np", toolName);
      const secondData = parseToolText<T | NpErrorResponse>(
        "gofr-np",
        toolName,
        secondText,
      );
      ...
      throw new ApiError({
        service: "gofr-np",
        tool: toolName,
        message: secondErr.detail
          ? `${secondErr.error}: ${secondErr.detail}`
          : secondErr.error,
...
class McpClient {
  ...
  private async doInitialize(): Promise<void> {
    ...
    const timer = setTimeout(
      () =>
        controller.abort(
          `${this.serviceName} initialize timed out after ${INIT_TIMEOUT_MS / 1000}s`,
        ),
      INIT_TIMEOUT_MS,
    );
  ...
  async callTool<T>(
    ...
    const timeout = setTimeout(
      () =>
        controller.abort(
          `${this.serviceName}/${toolName} timed out after 40 s`,
        ),
      40000,
    );
    ...
    try {
      ...
      throw new ApiError({
        ...
        tool: toolName,
        message:
          err instanceof Error ? err.message : "Failed to parse MCP response",
...
export const api = {
  ...
  npMathListOperations: async (): Promise<NpMathListOperationsResponse> => {
    return callNpToolWithAuthFallback<NpMathListOperationsResponse>(
  ...
  npMathCompute: async (
    args: Record<string, unknown>,
    authToken?: string,
  ): Promise<NpMathResult> => {
    return callNpToolWithAuthFallback<NpMathResult>(
      "math_compute",
      args,
      authToken,
    );
  ...
  npCurveFit: async (
    args: Record<string, unknown>,
    authToken?: string,
  ): Promise<NpCurveFitResponse> => {
    return callNpToolWithAuthFallback<NpCurveFitResponse>(
      "curve_fit",
      args,
      authToken,
    );
  ...
  npFinancialPv: async (
    args: Record<string, unknown>,
    authToken?: string,
  ): Promise<NpFinancialPvResponse> => {
    return callNpToolWithAuthFallback<NpFinancialPvResponse>(
      "financial_pv",
      args,
      authToken,
    );
  ...
  npFinancialConvertRate: async (
    args: Record<string, unknown>,
    authToken?: string,
  ): Promise<NpFinancialConvertRateResponse> => {
    return callNpToolWithAuthFallback<NpFinancialConvertRateResponse>(
  ...
  npFinancialOptionPrice: async (
    args: Record<string, unknown>,
    authToken?: string,
  ): Promise<NpFinancialOptionPriceResponse> => {
    return callNpToolWithAuthFallback<NpFinancialOptionPriceResponse>(
  ...
  npFinancialBondPrice: async (
    args: Record<string, unknown>,
    authToken?: string,
  ): Promise<NpFinancialBondPriceResponse> => {
    return callNpToolWithAuthFallback<NpFinancialBondPriceResponse>(
  ...
  npFinancialTechnicalIndicators: async (
    args: Record<string, unknown>,
    authToken?: string,
  ): Promise<NpFinancialTechnicalIndicatorsResponse> => {
    return callNpToolWithAuthFallback<NpFinancialTechnicalIndicatorsResponse>(
  ...
  docListTemplates: async (): Promise<DocListTemplatesResponse> => {
    ...
    return parseToolText<DocListTemplatesResponse>(
      "gofr-doc",
      "list_templates",
      textContent,
    );
  ...
  docGetTemplateDetails: async (
    ...
    return parseToolText<DocTemplateDetailsResponse>(
      "gofr-doc",
      "get_template_details",
      textContent,
    );
  ...
  docListTemplateFragments: async (
    ...
    return parseToolText<DocListTemplateFragmentsResponse>(
      "gofr-doc",
      "list_template_fragments",
      textContent,
    );
  ...
  docGetFragmentDetails: async (
    ...
    return parseToolText<DocFragmentDetailsResponse>(
      "gofr-doc",
      "get_fragment_details",
      textContent,
    );
  ...
  docListStyles: async (): Promise<DocListStylesResponse> => {
    ...
    return parseToolText<DocListStylesResponse>(
      "gofr-doc",
      "list_styles",
      textContent,
    );
  ...
  docCreateDocumentSession: async (
    ...
    return parseToolText<DocCreateSessionResponse>(
      "gofr-doc",
      "create_document_session",
      textContent,
    );
  ...
  docListActiveSessions: async (
    authToken: string,
  ): Promise<DocListActiveSessionsResponse> => {
    const client = getMcpClient("gofr-doc");
    const result = await client.callTool<HealthCheckResult>(
    ...
    return parseToolText<DocListActiveSessionsResponse>(
      "gofr-doc",
      "list_active_sessions",
      textContent,
    );
  ...
  docGetSessionStatus: async (
    authToken: string,
    sessionId: string,
  ): Promise<DocSessionStatusResponse> => {
    const client = getMcpClient("gofr-doc");
    const result = await client.callTool<HealthCheckResult>(
      "get_session_status",
      {
    ...
    return parseToolText<DocSessionStatusResponse>(
      "gofr-doc",
      "get_session_status",
      textContent,
    );
  ...
  docAbortDocumentSession: async (
    ...
    return parseToolText<DocAbortSessionResponse>(
      "gofr-doc",
      "abort_document_session",
      textContent,
    );
  ...
  docValidateParameters: async (args: {
    ...
    return parseToolText<DocValidateParametersResponse>(
      "gofr-doc",
      "validate_parameters",
      textContent,
    );
  ...
  docSetGlobalParameters: async (
    ...
    return parseToolText<DocSetGlobalParametersResponse>(
      "gofr-doc",
      "set_global_parameters",
      textContent,
    );
  ...
  docAddFragment: async (
    ...
    return parseToolText<DocAddFragmentResponse>(
      "gofr-doc",
      "add_fragment",
      textContent,
    );
  ...
  docAddImageFragment: async (
    ...
    return parseToolText<DocAddImageFragmentResponse>(
      "gofr-doc",
      "add_image_fragment",
      textContent,
    );
  ...
  docListSessionFragments: async (
    ...
    return parseToolText<DocListSessionFragmentsResponse>(
      "gofr-doc",
      "list_session_fragments",
      textContent,
    );
  ...
  docRemoveFragment: async (
    ...
    return parseToolText<DocRemoveFragmentResponse>(
      "gofr-doc",
      "remove_fragment",
      textContent,
    );
  ...
  docGetDocument: async (
    ...
    return parseToolText<DocGetDocumentResponse>(
      "gofr-doc",
      "get_document",
      textContent,
    );
  ...
  docPlotListThemes: async (): Promise<PlotListThemesResponse> => {
    ...
    return parseToolText<PlotListThemesResponse>(
      "gofr-doc",
      "list_themes",
      textContent,
    );
  ...
  docPlotListHandlers: async (): Promise<PlotListHandlersResponse> => {
    ...
    return parseToolText<PlotListHandlersResponse>(
      "gofr-doc",
      "list_handlers",
      textContent,
    );
  ...
  docPlotListImages: async (
    authToken: string,
  ): Promise<PlotListImagesResponse> => {
    const client = getMcpClient("gofr-doc");
    ...
    return parseToolText<PlotListImagesResponse>(
      "gofr-doc",
      "list_images",
      textContent,
    );
  ...
  docPlotGetImage: async (
    authToken: string,
    identifier: string,
  ): Promise<PlotGetImageResponse> => {
    const client = getMcpClient("gofr-doc");
    ...
    const meta = parseToolText<PlotGetImageResponse["meta"]>(
      "gofr-doc",
      "get_image",
      textContent,
    );
  ...
  docPlotRenderGraph: async (
    ...
    if (!hasImage) {
      const data = parseToolText<PlotRenderGraphProxyData>(
        "gofr-doc",
        "render_graph",
        textContent,
      );
    ...
    const meta = parseToolText<PlotRenderGraphInlineMeta>(
      "gofr-doc",
      "render_graph",
      textContent,
    );
  ...
  docPlotAddPlotFragment: async (
    ...
    return parseToolText<PlotAddPlotFragmentResponse>(
      "gofr-doc",
      "add_plot_fragment",
      textContent,
    );
  ...
  digSetAntiDetection: async (
    ...
    return parseToolText<AntiDetectionResponse>(
      "gofr-dig",
      "set_antidetection",
      textContent,
    );
  ...
  digGetStructure: async (
    ...
    return parseToolText<PageStructureResponse>(
      "gofr-dig",
      "get_structure",
      textContent,
    );
  ...
  digGetContent: async (
    ...
    return parseToolText<ContentResponse>(
      "gofr-dig",
      "get_content",
      textContent,
    );
  ...
  digGetSessionInfo: async (
    ...
    return parseToolText<SessionInfoResponse>(
      "gofr-dig",
      "get_session_info",
      textContent,
    );
  ...
  digGetSessionChunk: async (
    ...
    const parsed = parseToolText<SessionChunkResponse>(
      "gofr-dig",
      "get_session_chunk",
      textContent,
    );
  ...
  digGetSessionUrls: async (
    ...
    return parseToolText<SessionUrlsResponse>(
      "gofr-dig",
      "get_session_urls",
      textContent,
    );
  ...
  digGetSessionUrlsAsJson: async (
    ...
    const textContent = getTextContent(result, "gofr-dig", "get_session_urls");
    return parseToolText<SessionUrlsJsonResponse>(
      "gofr-dig",
      "get_session_urls",
      textContent,
    );
  ...
  digGetSession: async (
    ...
    return parseToolText<GetSessionResponse>(
      "gofr-dig",
      "get_session",
      textContent,
    );
  ...
  digListSessions: async (
    ...
    return parseToolText<ListSessionsResponse>(
      "gofr-dig",
      "list_sessions",
      textContent,
    );
  ...
  listSources: async (authToken: string): Promise<{ sources: Source[] }> => {
    ...
    return parseToolText("gofr-iq", "list_sources", textContent) as {
      sources: Source[];
    };
  },
  ...
  ingestDocument: async (
    ...
    const client = getMcpClient("gofr-iq");
    const result = await client.callTool<HealthCheckResult>("ingest_document", {
      title,
      content,
  ...
  listInstruments: async (
    ...
    try {
      ...
      return {
        instruments: Array.from(instrumentMap.values()) as Instrument[],
      };
    } catch (err) {
      logger.warn({
        event: "api_call_failed",
  ...
  getMarketContext: async (
    ...
    return parseToolText<MarketContextResponse>(
      "gofr-iq",
      "get_market_context",
      textContent,
    );
  ...
  listClients: async (
    ...
    const clients = (data.clients ?? []).map((c) => {
      ...
      return {
        ...
        portfolio_guid:
          typeof c.portfolio_guid === "string" ? c.portfolio_guid : undefined,
        watchlist_guid:
          typeof c.watchlist_guid === "string" ? c.watchlist_guid : undefined,
      } as ClientSummary;
    });
  ...
  getClientProfile: async (
    ...
    return parseToolText<ClientProfileResponse>(
      "gofr-iq",
      "get_client_profile",
      textContent,
    );
  ...
  getClientProfileScore: async (
    ...
    return parseToolText<ProfileScoreResponse>(
      "gofr-iq",
      "get_client_profile_score",
      textContent,
    );
  ...
  updateClientProfile: async (
    ...
    return parseToolText<UpdateProfileResponse>(
      "gofr-iq",
      "update_client_profile",
      textContent,
    );
  ...
  createClient: async (
    ...
    return parseToolText<CreateClientResponse>(
      "gofr-iq",
      "create_client",
      textContent,
    );
  ...
  getDocument: async (
    ...
    return parseToolText<DocumentResponse>(
      "gofr-iq",
      "get_document",
      textContent,
    );
  ...
  getClientFeed: async (
    ...
    return parseToolText<ClientFeedResponse>(
      "gofr-iq",
      "get_top_client_news",
      textContent,
    );
  ...
  whyItMattersToClient: async (
    ...
    return parseToolText<WhyItMattersToClientResponse>(
      "gofr-iq",
      "why_it_matters_to_client",
      textContent,
    );
  ...
  addToPortfolio: async (
    ...
    return parseToolText<PortfolioUpdateResponse>(
      "gofr-iq",
      "add_to_portfolio",
      textContent,
    );
  ...
  getPortfolioHoldings: async (
    ...
    return parseToolText<PortfolioHoldingsResponse>(
      "gofr-iq",
      "get_portfolio_holdings",
      textContent,
    );
  ...
  addToWatchlist: async (
    ...
    return parseToolText<WatchlistUpdateResponse>(
      "gofr-iq",
      "add_to_watchlist",
      textContent,
    );
  ...
  getClientWatchlist: async (
    ...
    return parseToolText<WatchlistResponse>(
      "gofr-iq",
      "get_watchlist_items",
      textContent,
    );
  ...
  getInstrumentNews: async (
    ...
    const client = getMcpClient("gofr-iq");
    const defaultResponse: InstrumentNewsResponse = {
      ticker,
      articles: [],
      total_found: 0,
    };
    ...
    try {
      ...
      return parseToolText<InstrumentNewsResponse>(
        "gofr-iq",
        "get_instrument_news",
        textContent,
      );
