...
### Step 1: Data Types & Constants (30 min)
**File:** `src/types/restrictions.ts` (NEW)
- Define TypeScript interfaces matching backend schema
- Export constants for EXCLUDED_INDUSTRIES and IMPACT_THEMES
- Create helper functions for validation
- Add restrictions field to ClientProfile interface in `src/types/clientProfile.ts`

### Step 2: Restrictions Editor Component (1-2 hours)
**File:** `src/components/client/RestrictionsEditor.tsx` (NEW)
- Multi-checkbox group for excluded industries (TOBACCO, WEAPONS, etc.)
- Select dropdown for faith_based compliance
- Toggle switch for impact_mandate
- Multi-select chip component for impact_themes
- Toggle switch for stewardship_obligations
- Collapsed/disabled sections for future fields (legal, operational, tax)
- Props: value (restrictions), onChange, disabled

### Step 3: Restrictions Summary Component (30 min)
**File:** `src/components/client/RestrictionsChip.tsx` (NEW)
- Compact visual indicator showing restriction count
- Tooltip with full restriction details
- Color-coded badges (red for exclusions, green for impact)
- Click handler to expand full view

### Step 4: Extend Profile Edit Dialog (45 min)
**File:** `src/components/client/ClientProfileEditDialog.tsx` (MODIFY)
- Add new "Restrictions" tab using Material-UI Tabs
- Integrate RestrictionsEditor component
- Handle restrictions state management
- Update API call to include restrictions in payload
- Validate restrictions before save
- Handle full replacement semantics (warn user about overwrite)

### Step 5: Update Client Header (30 min)
**File:** `src/components/common/ClientHeader.tsx` (MODIFY)
- Add RestrictionsChip below client name/type
- Show badges: "🚫 3 exclusions", "🌱 2 themes", "☪️ Shariah"
- Only show if restrictions exist
- Click to open profile editor on Restrictions tab

### Step 6: News Feed Filter Transparency (30 min)
**File:** `src/components/common/ClientNewsPanel.tsx` (MODIFY)
- Add info alert showing filtered article count
- Break down by exclusion reason (e.g., "Hiding 3 TOBACCO, 9 FOSSIL_FUELS")
- "Show All Anyway" toggle to temporarily bypass filters
- Visual indicator on filtered articles

### Step 7: Client Creation Wizard Extension (45 min)
**File:** `src/components/client/ClientCreateWizard.tsx` (NEW or MODIFY)
- Add Step 4: "ESG & Compliance Restrictions"
- Quick template buttons (No Restrictions, ESG Screened, Shariah, Impact Fund)
- Include RestrictionsEditor for custom configuration
- Pre-populate based on template selection

### Step 8: Restriction Templates (15 min)
**File:** `src/utils/restrictionTemplates.ts` (NEW)
- Export preset configurations:
  - `TEMPLATE_ESG_SCREENED`: Common exclusions (tobacco, weapons, fossil fuels)
  - `TEMPLATE_SHARIAH`: Shariah-compliant exclusions + faith_based
  - `TEMPLATE_IMPACT_FUND`: Impact themes + stewardship obligations
  - `TEMPLATE_UCITS`: Legal/operational constraints
- Helper function to apply template to restrictions object

### Step 9: API Integration (30 min)
**File:** `src/services/api/index.ts` (MODIFY)
- Ensure createClient supports restrictions parameter
- Ensure updateClientProfile supports restrictions parameter
- Handle restrictions in getClientProfile response parsing
- Add type safety for restrictions payloads

### Step 10: Testing & Polish (1 hour)
- Test full CRUD flow for restrictions
- Test template application
- Verify news feed filtering works correctly
- Test backwards compatibility (clients without restrictions)
