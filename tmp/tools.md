...
Complete reference for all MCP tools provided by gofr-dig.
...
| session | bool | No | false | Save to a session, return `session_id` instead of full content |
...
Root page fields plus:

```json
{
  "pages": [
    {"depth": 1, "url": "https://docs.example.com/start", "title": "Getting Started", "text": "..."}
  ],
  "summary": {
    "total_pages": 6,
...
3. **`get_content`** — extract text (use `session=true` for large results)
4. **`get_session_urls`** — get chunk URLs for automation fan-out
5. **`get_session_info` / `get_session_chunk`** — paginate through large results
6. **`list_sessions`** — browse all stored sessions

### Tips

- Start with `depth=1` before going deeper
- Use `get_structure` to find good CSS selectors before `get_content`
- Keep `max_pages_per_level` low initially to avoid rate limiting
- Use `session=true` for large crawls to avoid payload limits
